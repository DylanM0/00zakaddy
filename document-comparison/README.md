...
이 도구는 **간단하고 직관적인 문자 기반 유사도**를 사용합니다:

**LCS (Longest Common Subsequence)** 알고리즘을 사용하여 두 문장 간 공통으로 일치하는 글자 수를 계산합니다.

### 계산 방식
```
유사도 = (일치하는 글자 수 / 전체 글자 수) × 100%
```

### 예시
- 원본: "안녕하세요 반갑습니다" (11글자)
- 비교: "안녕하세요 감사합니다" (11글자)
- 일치하는 글자: "안녕하세요" + "니다" = 8글자
- **유사도: 8/11 × 100 = 72.7%**

### 파스텔톤 색상 시스템
유사도에 따라 직관적인 파스텔 색상으로 표시됩니다:
- 🟢 **95-100%**: 진한 초록 파스텔 (거의 동일)
- 🟢 **85-94%**: 밝은 초록 파스텔 (매우 유사)
- 🟡 **75-84%**: 연두 파스텔 (유사)
- 🟡 **65-74%**: 노랑 파스텔 (약간 유사)
- 🟠 **55-64%**: 주황 파스텔 (다소 차이)
- 🔴 **50-54%**: 빨강 파스텔 (많이 다름)
...
### 📊 직관적인 시각화
- **파스텔톤 색상 시스템**: 10% 간격으로 구분되는 부드러운 색상
- **클릭 인터랙션**: 원본 문장 클릭 시 비교 문서의 해당 위치로 자동 스크롤
- **호버 툴팁**: 마우스 오버 시 어떤 문서의 몇 번째 문장과 몇 % 유사한지 표시
- **나란히 비교**: 화면을 최대한 활용한 반응형 레이아웃

### 🔍 상세 분석
- **글자 수 기반**: 전체 글자 대비 일치하는 글자 수로 명확한 유사도 계산
- **단어 단위 추적**: 같은 단어 / 추가된 단어 / 제거된 단어를 색상으로 구분
- **실시간 하이라이트**: 선택한 문장의 차이점을 즉시 시각화

### ⚡ 강력한 기능
- **다양한 파일 형식**: PDF, Word, Excel, CSV, TXT 지원
- **대용량 지원**: 최대 50MB 파일 업로드
- **빠른 처리**: 실시간 분석 및 결과 표시
- **한글 완벽 지원**: 한국어 문서 최적화
